...
export default function TaskModal({
  ...
  // Photos can arrive three ways: the file picker inside TaskImagesField, a
  // drag onto anywhere in this modal, or a pasted screenshot.
  const setImageUrls = useCallback((urls: string[] | null) => {
    setForm((f) => ({ ...f, image_urls: urls }));
  }, []);
  const images = useTaskImages({ value: form.image_urls, taskId: form.id, onChange: setImageUrls });
  const { isDraggingImage, dropHandlers } = useImageDropTarget({ onTransfer: images.addFromTransfer });
  ...
  return (
    <div
      ...
      {...dropHandlers}
    >
      {/* The whole backdrop accepts drops so the target is forgiving, but the
          hint hugs the card so it reads as "attaches to this ticket". */}
      <div className="relative w-full max-w-lg">
        {isDraggingImage && (
          <div className="pointer-events-none absolute inset-0 z-10 flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-blue-500 bg-gray-950/85 backdrop-blur-[2px]">
            ...
            <p className="text-sm font-medium text-blue-300">Drop image to attach</p>
            <p className="text-xs text-blue-300/60">It uploads straight to this ticket</p>
          </div>
        )}
        ...
        <form
          ...
          className="bg-gray-900 border border-gray-700 rounded-xl w-full max-h-[90vh] overflow-y-auto p-6"
        >
